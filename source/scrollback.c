 ...
 * starting at q_scrollback_buffer.  However, the behavior of that list
 * is a little unusual:
 *
 ...
 * Q_STATE_CONSOLE state I want to make sure the last line is still
 * visible directly above the status line(s).
 *
 ...
 *
 * I almost want to add a variable origin (0,0) so that the screen
 * could pan left to right.  But I don't know any of any terminals
 * that require that.  Still, it would bring new life to "scroll
 * lock".  :)
 */
...
/* The current edit line of the scrollback buffer. */
struct q_scrolline_struct * q_scrollback_current = NULL;

/* The current view position in the scrollback buffer.
   This position is the BOTTOM on the screen, so when we render we
   count from here and go UP until we run out of available rows
   to render to. */
struct q_scrolline_struct * q_scrollback_position = NULL;

/* The maximum size of the scrollback buffer. Default 20000. */
...
/*
 * Find the scrollback line that corresponds to the top line
 * of the screen.
 */
static struct q_scrolline_struct * find_top_scrollback_line() {
        ...
        /* Let's assert that row > 0.  Konsole and xterm won't
           let the window size reach zero so this should be a
           non-issue. */
        assert(row > 0);
...
/*
 * insert_scrollback_line - initialize a new line for the scrollback
 * buffer.  The line is inserted before insertion_point.
 */
static void insert_scrollback_line(struct q_scrolline_struct * insertion_point) {
        struct q_scrolline_struct * new_line;
        int i;

        assert (insertion_point != NULL);

        new_line = (struct q_scrolline_struct *)Xmalloc(sizeof(struct q_scrolline_struct), __FILE__, __LINE__);
        ...
        if (q_scrollback_buffer == NULL) {
                ...
                new_line->prev = insertion_point->prev;
                new_line->next = insertion_point;
                insertion_point->prev = new_line;
                /*
                 * ASCII downloads and the console itself both update the
                 * scrollback and need to render the new line.
                 */
        ...
        if (    ((q_scrollback_max > 0) && (q_status.scrollback_lines >= q_scrollback_max)) ||
                ((q_status.scrollback_enabled == Q_FALSE) && (q_status.scrollback_lines > HEIGHT - STATUS_HEIGHT - 1))
        ) {
...
/*
 * new_scrollback_line - initialize a new line for the scrollback
 * buffer.  The line is appended to the buffer, becoming the new
 * q_scrollback_last.
 */
void new_scrollback_line() {
        ...
        if (q_status.emulation == Q_EMUL_DEBUG) {
                /* DEBUG emulation plays tricks with the scrollback buffer.
                 * If I don't explicitly set the color the cursor will
                 * disappear.
                 */
                for (i = 0; i < Q_MAX_LINE_LENGTH; i++) {
                        new_line->colors[i] = Q_A_REVERSE | scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
                }
        }
...
/* Special case for VT100 line wrapping. */
...
void print_character(const wchar_t character) {
        ...
        int i;
        /*
         * I want a const character in the API, but it's convenient
         * for flow control to change character.
         */
        ...
        if (q_scrollback_current->length < q_status.cursor_x) {
                for (i = q_scrollback_current->length; i < q_status.cursor_x; i++) {
                        ...
                        q_scrollback_current->colors[i] = scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
        ...
        if (q_status.cursor_x == right_margin) {
                ...
                /* This case happens when:  the cursor was already
                   on the right margin (either through printing or by
                   an explicit placement command), and a character
                   was printed. */
                ...
                if ((q_status.emulation == Q_EMUL_VT100) ||
                    ...
                    (q_status.emulation == Q_EMUL_XTERM_UTF8)
                        ...
                        /* Special case for VT100: the line wraps
                           only when a new character arrives AND the cursor
                           is already on the right margin AND has placed
                           a character in its cell.  Easier to see than
                           to explain. */
                        ...
                        if (q_status.line_wrap == Q_TRUE) {
                                ...
                                if (vt100_wrap_line_flag == Q_FALSE) {
                                        ...
                                        /* This block marks the case that we are
                                           in the margin and the second character
                                           has been received and printed. */
                        ...
                        if (q_status.line_wrap == Q_TRUE) {
                                /* Regular NON-VT100 case.  The cursor moves
                                   to the first column of the next line as soon
                                   as the character has been printed in the
                                   right margin column. */
                                 ...
                                 * Line wrap is OFF so do not wrap the line.
                                 *
                                 * Also, Keep the cursor where it is on the
                                 * right margin.
                                 */
                                wrap_the_line = Q_FALSE;
                ...
                /* This is the normal case: a character came in and
                   was printed to the left of the right margin column. */
                wrap_the_line = Q_FALSE;
        ...
        if (q_status.cursor_x < q_scrollback_current->length) {
                ...
                q_scrollback_current->colors[q_scrollback_current->length] = q_current_color;
        ...
        if (q_status.capture == Q_TRUE) {
                ...
                if (q_status.capture_type == Q_CAPTURE_TYPE_HTML) {
                        ...
                        if (color_changed == Q_TRUE) {
                                fprintf(q_status.capture_file, "</font><font %s>", color_to_html(q_current_color));
                        }
                        if (character2 == ' ') {
                                fprintf(q_status.capture_file, "&nbsp;");
                        } else if (character2 == '<') {
...
/*
 * Save one line of the visible scrollback to file, including HTML or NORMAL mode
 */
static void save_scrollback_line(FILE * file, struct q_scrolline_struct * line, Q_CAPTURE_TYPE save_type, attr_t * last_color) {
        int i;
        wchar_t ch;
        Q_BOOL color_changed = Q_FALSE;
        ...
        for (i = 0; i < WIDTH; i++) {
                ...
                if (i >= line->length) {
                        ch = ' ';
                        if (*last_color != (Q_A_NORMAL | scrollback_full_attr(Q_COLOR_CONSOLE_TEXT))) {
                                *last_color = Q_A_NORMAL | scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
                                color_changed = Q_TRUE;
                        }
                } else {
                        ch = line->chars[i];
 ...
 * save_scrollback
 */
static Q_BOOL save_scrollback(const char * filename, const Q_BOOL visible_only) {
        struct q_scrolline_struct * line;
        FILE * file;
        time_t current_time;
        ...
        if (q_status.scrollback_save_type == Q_CAPTURE_TYPE_HTML) {
                /* HTML */
                strftime(time_string, sizeof(time_string), _("Saved Scrollback Generated %a, %d %b %Y %H:%M:%S %z"), localtime(&current_time));
                fprintf(file, "<html>\n\n");
                fprintf(file, "<!-- * - * Qodem " Q_VERSION " %s BEGIN * - * --> \n\n", time_string);
                fprintf(file, "<body bgcolor=\"black\">\n<pre {font-family: 'Courier New', monospace;}><code><font %s>", color_to_html(q_current_color));
        } else {
                strftime(time_string, sizeof(time_string), _("Saved Scrollback Generated %a, %d %b %Y %H:%M:%S %z"), localtime(&current_time));
                fprintf(file, "* - * Qodem " Q_VERSION " %s BEGIN * - *\n\n", time_string);
        }
        ...
        if (visible_only == Q_TRUE) {
                ...
                while ((row < HEIGHT - STATUS_HEIGHT) && (line != NULL)) {
                        save_scrollback_line(file, line, q_status.scrollback_save_type, &color);
                        line = line->next;
                        row++;
                }
        } else {
...
Q_BOOL screen_dump(const char * filename) {
        ...
        if (q_status.screen_dump_type == Q_CAPTURE_TYPE_HTML) {
                /* HTML */
                strftime(time_string, sizeof(time_string), _("Screen Dump Generated %a, %d %b %Y %H:%M:%S %z"), localtime(&current_time));
                fprintf(file, "<html>\n\n");
                fprintf(file, "<!-- * - * Qodem " Q_VERSION " %s BEGIN * - * --> \n\n", time_string);
                fprintf(file, "<body bgcolor=\"black\">\n<pre {font-family: 'Courier New', monospace;}><code><font %s>", color_to_html(q_current_color));
        } else {
...
void scrollback_keyboard_handler(const int keystroke, const int flags) {
        ...
        case 'F':
                ...
                for (i = 0; i < wcslen(q_scrollback_search_string); i++) {
                        q_scrollback_search_string[i] = towlower(q_scrollback_search_string[i]);
                }
                ...
                while (line != NULL) {
                        ...
                        if (wcsstr(lower_line, q_scrollback_search_string) != NULL) {
                                ...
                                while (begin != NULL) {
                                        for (i = 0; i < wcslen(q_scrollback_search_string); i++) {
                                                line->search_colors[i + (begin - lower_line)] |= Q_A_BLINK | Q_A_REVERSE;
                                        }
        ...
        keep_moving:
                ...
                if (line == NULL) {
                        ...
                        /*
                         * Next time, show the matches from the first
                         * line.
                         */
                        last_line = q_scrollback_buffer;
                        break;
        ...
        case Q_KEY_UP:
                /* Make sure we need to scroll. This is pretty ugly but
                 it works so I won't bugger with it anymore. Same code
                 is in the KEY_PPAGE case too but j instead of i. */
                ...
                if ((q_scrollback_position != NULL) && (q_scrollback_position->prev != NULL)) {
                        q_scrollback_position = q_scrollback_position->prev;
                }
                break;

        case Q_KEY_DOWN:
                if ((q_scrollback_position != NULL) && (q_scrollback_position->next != NULL)) {
                        q_scrollback_position = q_scrollback_position->next;
                }
                break;
...
/* Console needs to know because it places the cursor */
...
void render_scrollback(const int skip_lines) {
        ...
        if (row < 0) {
                ...
                fprintf(stderr, "render_scrollback() WARNING: Screen size is too small: height=%d width=%d status_height=%d skip_lines=%d\n", HEIGHT, WIDTH, STATUS_HEIGHT, skip_lines);
                return;
        }
        ...
        /* At this point line should point to the line directly
         * above find_top_scrollback_line() IF row is -1.
         * If row is 0 or more then the scrollback buffer is not
                ...
                if ((line->dirty == Q_TRUE) || Q_TRUE) {
                        ...
                        if (line->length > 0) {
                                for (i = 0; i < line->length; i++) {
                                        ...
                                        color = vt100_check_reverse_color(color, line->reverse_color);
                                        ...
                                        if (line->double_width == Q_TRUE) {
                                                ...
                                                if (xterm == Q_FALSE) {
                                                        screen_put_scrollback_char_yx(row, (2 * i), line->chars[i], color);
                                                        screen_put_scrollback_char_yx(row, (2 * i) + 1, ' ', color);
                                                } else {
                                                        screen_put_scrollback_char_yx(row, i, line->chars[i], color);
                                                }
...
void scrollback_refresh() {
        ...
        scrollback_string = _(" SCROLL-BACK    F/A-Find  S/T-Save All/Screen  C-Clear  ESC/`-Exit  Lines:");
        ...
        screen_put_color_printf_yx(HEIGHT - 1, status_left_stop, Q_COLOR_STATUS, "%s %s ", scrollback_string, lines_count_buffer);

        /* Add arrows on the status line */
        if (q_scrollback_position != q_scrollback_last) {
                /* Down arrow - more lines are below */
                screen_put_color_char_yx(HEIGHT - 1, status_left_stop + 14, cp437_chars[DOWNARROW], Q_COLOR_STATUS);
        }
        if (find_top_scrollback_line() != q_scrollback_buffer) {
                /* Up arrow - more lines are above */
                screen_put_color_char_yx(HEIGHT - 1, status_left_stop + 13, cp437_chars[UPARROW], Q_COLOR_STATUS);
        }
...
/*
 * Scroll a region between region_top and region_bottom, inclusive, up
 * by count lines.  The size of the region is (region_bottom + 1 -
 * region_top) lines, because region_bottom is the row number of the
 * bottom line.
 */
void rectangle_scroll_up(const int top, const int left, const int bottom, const int right, const int count) {
        struct q_scrolline_struct * top_line;
        ...
        for (i = 0; i < remaining; i++) {
                if (new_top_line->length < top_line->length) {
                        for (j = new_top_line->length; j < right; j++) {
                                ...
                                new_top_line->colors[j] = scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
                                new_top_line->length = right;
                        }
                }
                memcpy(new_top_line->chars + left, top_line->chars + left, sizeof(wchar_t) * (right - left));
                memcpy(new_top_line->colors + left, top_line->colors + left, sizeof(attr_t) * (right - left));
                ...
                if (top_line == NULL) {
                        /*
                         * We are scrolling up on the very first screen and
                         * lines don't exist for the remainder of the scroll
                         * operation.  Just break out and let erase_screen()
                         * take care of things.
                         */
                        break;
                }
...
/*
 * Scroll the entire screen up count lines
 */
void scroll_up(const int count) {
        scrolling_region_scroll_up(0, HEIGHT - STATUS_HEIGHT - 1, count);
} /* ---------------------------------------------------------------------- */

void scrolling_region_scroll_up(const int region_top, const int region_bottom, const int count) {
        rectangle_scroll_up(region_top, 0, region_bottom, Q_MAX_LINE_LENGTH, count);
} /* ---------------------------------------------------------------------- */

/*
 * Scroll a region between region_top and region_bottom, inclusive,
 * down by count lines.  The size of the region is (region_bottom + 1
 * - region_top) lines, because region_bottom is the row number of the
 * bottom line.
 */
void rectangle_scroll_down(const int top, const int left, const int bottom, const int right, const int count) {
        ...
        for (i = 0; i < remaining; i++) {
                if (new_bottom_line->length < left) {
                        for (j = new_bottom_line->length; j < right; j++) {
                                ...
                                new_bottom_line->colors[j] = scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
                                new_bottom_line->length = right;
                        }
                }
                memcpy(new_bottom_line->chars + left, bottom_line->chars + left, sizeof(wchar_t) * (right - left));
                memcpy(new_bottom_line->colors + left, bottom_line->colors + left, sizeof(attr_t) * (right - left));
...
void scrolling_region_scroll_down(const int region_top, const int region_bottom, const int count) {
        rectangle_scroll_down(region_top, 0, region_bottom, Q_MAX_LINE_LENGTH, count);
} /* ---------------------------------------------------------------------- */

/*
 * Scroll the entire screen down count lines
 */
                ...
                if (q_status.cursor_y < bottom) {
                        ...
                        if (q_scrollback_current->next == NULL) {
                                ...
                                for (j=q_status.cursor_x; j>q_scrollback_current->length;) {
                                        q_scrollback_last->colors[q_scrollback_last->length] = scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
...
/*
 * Replace the characters in the current line from the start column (from 0
 * to WIDTH-1) to the end column (from 0 to WIDTH-1), inclusive.
 */
void fill_line_with_character(const int start, const int end, const wchar_t character, const Q_BOOL honor_protected) {
        int i;
        ...
        for (i = q_scrollback_current->length; i < start; i++) {
                ...
                case Q_EMUL_VT220:
                         ...
                         *
                         * Erasing a character also erases any
                         * character attribute of the character.
                         */
                        q_scrollback_current->colors[i] = scrollback_full_attr(Q_COLOR_CONSOLE_TEXT);
                        break;
                default:
                        /* Most other consoles erase with the current color a.k.a. back-color erase (bce). */
        ...
        for (i = start; i <= end; i++) {
                if ((honor_protected == Q_FALSE) || ((honor_protected == Q_TRUE) && ((q_scrollback_current->colors[i] & Q_A_PROTECT) == 0))) {
                        q_scrollback_current->chars[i] = character;
        ...
        /* If we erased beyond the end of the line, increase the line length
           to include the new spaces. */
        if (end > q_scrollback_current->length) {
                q_scrollback_current->length = end + 1;
        }

        /* If the line is now longer than the screen, shorten it to WIDTH
           or else we'll have problems with line wrapping. */
...
/*
 * erase_line - erase the characters in the current line from the
 * start column (from 0 to WIDTH-1) to the end column (from 0 to
 * WIDTH-1), inclusive.
 */
...
/*
 * erase_screen - erase a rectangular section of the screen, inclusive.
 */
void erase_screen(const int start_row, const int start_col, const int end_row, const int end_col, const Q_BOOL honor_protected) {
        ...
        for (i = start_row; i <= end_row; i++) {
                ...
                /* Note: we don't add a line when (i == end_row)
                   because if end_row is the last line in scrollback
                   new_scrollback_line() will make the total screen
                   one line larger than it really is.  This causes
                   lots of trouble and looks like crap. */
...
void cursor_formfeed() {
        ...
        /*
         * Print the remaining number of linefeeds to clear the screen,
         * then home the cursor.
         */
...
void cursor_linefeed(const Q_BOOL new_line_mode) {
        ...
        if (q_status.cursor_y < q_status.scroll_region_bottom) {
                ...
                /* Two cases:  either we're inside a scrolling region
                   or not.  If the scrolling region bottom is the bottom
                   of the screen, then push the top line into the buffer.
                   Else scroll the scrolling region up. */
                if ((q_status.scroll_region_bottom == HEIGHT - STATUS_HEIGHT - 1) && (q_status.scroll_region_top == 0)) {

                        /* We're at the bottom of the scroll region, AND the
                           scroll region is the entire screen. */
                        ...
                        if (q_status.reverse_video == Q_TRUE) {
                                /* Increase width and pad with spaces
                                   so that inverse video appears
                                   everwhere. */
                                q_scrollback_current->length = WIDTH;
                        }
                        ...
                        /* We're at the bottom of the scroll region, AND the
                           scroll region is NOT the entire screen. */
...
void delete_character(const int count) {
        ...
        for (i = 0; i < count; i++) {
                ...
                /* cursor_x and cursor_y don't change. We just copy
                   the existing line leftwise one char. */
...
void insert_blanks(const int count) {
        ...
        /* cursor_x and cursor_y don't change. We just copy the
         existing line rightwise one char. */
...
/*
 * Flip the ACS_REVERSE toggle on every character in the visible
 * portion of the scrollback buffer.
 */
...
/*
 * Set double_width for this line.  It immediately sets single height.
 */
...
/*
 * Set double_height for this line.  It immediately sets double-width.
 */
