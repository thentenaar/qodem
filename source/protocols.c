...
/* Where transferred files are heading to/from */
 ...
 * ------------------------------------------------------------------------
 *
 * Rather than spill to a seperate module, it's so simple I'll just
 * leave it here.
 */
...
static ASCII_XFER_CRLF_POLICY ascii_xfer_upload_cr_handling = ASCII_XFER_CRLF_NONE;
static ASCII_XFER_CRLF_POLICY ascii_xfer_upload_lf_handling = ASCII_XFER_CRLF_NONE;
static ASCII_XFER_CRLF_POLICY ascii_xfer_download_cr_handling = ASCII_XFER_CRLF_NONE;
static ASCII_XFER_CRLF_POLICY ascii_xfer_download_lf_handling = ASCII_XFER_CRLF_NONE;

/*
 * Perform CRLF handling on a buffer.
 */
static void ascii_transfer_crlf_handling(const unsigned char * input, const int input_n, unsigned char * output, int * output_n, const ASCII_XFER_CRLF_POLICY cr_policy, const ASCII_XFER_CRLF_POLICY lf_policy) {

        int i, j;

        /* Check if we need to do anything */
        if ((cr_policy == ASCII_XFER_CRLF_NONE) && (lf_policy == ASCII_XFER_CRLF_NONE)) {
                /* Nothing to do */
...
/*
 * Perform an ASCII file transfer
 */
static void ascii_transfer(unsigned char * input, const int input_n, int * remaining, unsigned char * output, int * output_n, const int output_max) {
        ...
        if (ascii_xfer_sending == Q_TRUE) {
                ...
                ascii_transfer_crlf_handling(working_buffer, working_buffer_n, output, output_n, ascii_xfer_upload_cr_handling, ascii_xfer_upload_lf_handling);
                ...
                if (ferror(ascii_xfer_file)) {
                        /* Error */
                        char notify_message[DIALOG_MESSAGE_SIZE];
                        snprintf(notify_message, sizeof(notify_message), _("Error reading from file \"%s\": %s"), ascii_xfer_filename, strerror(errno));
                ...
                /* Perform CRLF handling. */
                ascii_transfer_crlf_handling(input, input_n, working_buffer, &working_buffer_n, ascii_xfer_download_cr_handling, ascii_xfer_download_lf_handling);
                ...
                if (ferror(ascii_xfer_file)) {
                        snprintf(notify_message, sizeof(notify_message), _("Error writing to file \"%s\": %s"), ascii_xfer_filename, strerror(errno));
                        notify_form(notify_message, 0);
...
/*
 * Setup the ASCII protocol for a file transfer
 */
static Q_BOOL ascii_transfer_start(const char * in_filename, const Q_BOOL send) {
        struct stat fstats;
        ...
        /* Pull the options */
        if (strcasecmp(get_option(Q_OPTION_ASCII_UPLOAD_USE_TRANSLATE_TABLE), "true") == 0) {
        ...
        if (strcasecmp(get_option(Q_OPTION_ASCII_DOWNLOAD_USE_TRANSLATE_TABLE), "true") == 0) {
                ascii_xfer_download_use_xlate_table = Q_TRUE;
        }
        if (strcasecmp(get_option(Q_OPTION_ASCII_DOWNLOAD_CR_POLICY), "strip") == 0) {
        ...
        if (strcasecmp(get_option(Q_OPTION_ASCII_DOWNLOAD_LF_POLICY), "strip") == 0) {
...
void set_transfer_stats_protocol_name(const char * new_string) {
        /*
         * Do nothing if being set to what it already is.
         * Note this is a pointer comparison, not a string
         * comparison.
         */
...
void set_transfer_stats_last_message(const char * format, ...) {
        /*
         * Since I always generate the string here I don't
         * need to check that I'm about to Xfree() myself.
         */
...
/*
 * Process incoming data within a file transfer
 */
void protocol_process_data(unsigned char * input, const int input_n, int * remaining, unsigned char * output, int * output_n, const int output_max) {

        if ((q_transfer_stats.state == Q_TRANSFER_STATE_ABORT) || (q_transfer_stats.state == Q_TRANSFER_STATE_END)) {
                return;
        }
        ...
        case Q_PROTOCOL_KERMIT:
                /*
                 * Kermit - I don't need to pass remaining because
                 * kermit.c does its own buffering in packet_buffer .
                 */
        ...
        case Q_PROTOCOL_YMODEM_G:
                /*
                 * Yes, ALL of these protocols use the same function to
                 * transmit data.
                 */
        ...
        case Q_PROTOCOL_ZMODEM:
                /*
                 * Zmodem - I don't need to pass remaining because
                 * zmodem.c does its own buffering in packet_buffer .
                 */
...
/*
 * clear_stats - reset transfer statistics
 */
static void clear_stats() {
        ...
        if (q_program_state == Q_STATE_UPLOAD_BATCH) {
                for (i=0;; i++) {
                        ...
                        q_transfer_stats.batch_bytes_total += batch_upload_file_list[i].fstats.st_size;
                }
        }
...
/*
 * start_file_transfer - Transfer a file between q_child_tty_fd and q_download_location.
...
void start_file_transfer() {
        ...
        case Q_PROTOCOL_ASCII:
                if (ascii_transfer_start(q_download_location, ((q_program_state == Q_STATE_UPLOAD) ? Q_TRUE : Q_FALSE)) == Q_FALSE) {
                ...
                break;
        case Q_PROTOCOL_KERMIT:
                if (kermit_start(batch_upload_file_list, q_download_location, ((q_program_state == Q_STATE_DOWNLOAD) ? Q_FALSE : Q_TRUE)) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console or host mode. */
                        /* Switch back to TERMINAL mode  or host mode */
                        if ((original_state == Q_STATE_HOST) || (original_state == Q_STATE_CONSOLE)) {
                ...
                break;
        case Q_PROTOCOL_XMODEM:
                if (xmodem_start(q_download_location, ((q_program_state == Q_STATE_UPLOAD) ? Q_TRUE : Q_FALSE), X_NORMAL) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console or host mode. */
                        /* Switch back to TERMINAL mode  or host mode */
                        if ((original_state == Q_STATE_HOST) || (original_state == Q_STATE_CONSOLE)) {
                                switch_state(original_state);
                ...
                break;
        case Q_PROTOCOL_XMODEM_CRC:
                if (xmodem_start(q_download_location, ((q_program_state == Q_STATE_UPLOAD) ? Q_TRUE : Q_FALSE), X_CRC) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console. */
                        switch_state(Q_STATE_CONSOLE);
                        return;
                }
                break;
        case Q_PROTOCOL_XMODEM_RELAXED:
                if (xmodem_start(q_download_location, ((q_program_state == Q_STATE_UPLOAD) ? Q_TRUE : Q_FALSE), X_RELAXED) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console. */
                        switch_state(Q_STATE_CONSOLE);
                        return;
                }
                break;
        case Q_PROTOCOL_XMODEM_1K:
                if (xmodem_start(q_download_location, ((q_program_state == Q_STATE_UPLOAD) ? Q_TRUE : Q_FALSE), X_1K) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console. */
                        switch_state(Q_STATE_CONSOLE);
                        return;
                }
                break;
        case Q_PROTOCOL_XMODEM_1K_G:
                if (xmodem_start(q_download_location, ((q_program_state == Q_STATE_UPLOAD) ? Q_TRUE : Q_FALSE), X_1K_G) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console. */
                        switch_state(Q_STATE_CONSOLE);
                        return;
                }
                break;
        case Q_PROTOCOL_YMODEM:
                if (ymodem_start(batch_upload_file_list, q_download_location, ((q_program_state == Q_STATE_UPLOAD_BATCH) ? Q_TRUE : Q_FALSE), Y_NORMAL) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console or host mode. */
                        /* Switch back to TERMINAL mode  or host mode */
                        if ((original_state == Q_STATE_HOST) || (original_state == Q_STATE_CONSOLE)) {
                                switch_state(original_state);
                ...
                break;
        case Q_PROTOCOL_YMODEM_G:
                if (ymodem_start(batch_upload_file_list, q_download_location, ((q_program_state == Q_STATE_DOWNLOAD) ? Q_FALSE : Q_TRUE), Y_G) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console. */
                        switch_state(Q_STATE_CONSOLE);
                        return;
                }
        ...
        case Q_PROTOCOL_ZMODEM:
                ...
                if (zmodem_start(batch_upload_file_list, q_download_location, ((q_program_state == Q_STATE_DOWNLOAD) ? Q_FALSE : Q_TRUE), Z_CRC32) == Q_FALSE) {
                        /* Couldn't start the protocol, switch back to console or host mode. */
                        /* Switch back to TERMINAL mode  or host mode */
                        if ((original_state == Q_STATE_HOST) || (original_state == Q_STATE_CONSOLE)) {
                                switch_state(original_state);
...
/*
 * stop_file_transfer - Stop a running transfer
 */
        ...
        case Q_PROTOCOL_XMODEM_1K_G:
                ...
                if (q_program_state == Q_STATE_DOWNLOAD) {
                        if (new_state == Q_TRANSFER_STATE_ABORT) {
                                ...
                                qlog(_("DOWNLOAD FILE COMPLETE: protocol %s, filename %s, filesize %d\n"), q_transfer_stats.protocol_name, q_download_location, q_transfer_stats.bytes_total);
                        }
                } else {
                        if (new_state == Q_TRANSFER_STATE_ABORT) {
...
void protocol_menu_refresh() {
        char * status_string;
        /* Number of digits in the hard drive size display.  10^32 is pretty darn big. */
        char size_string[32];
        ...
        status_string = _(" LETTER-Select a Protocol for the File Transfer   ESC/`-Exit ");
        ...
        if (q_program_state == Q_STATE_DOWNLOAD_MENU) {
                ...
                free_kbytes = (unsigned long long)buf.f_bavail * (unsigned long long)buf.f_bsize / 1024LL;
        ...
        i = 1;
        if (q_program_state == Q_STATE_DOWNLOAD_MENU) {
                screen_put_color_str_yx(window_top + 1, window_left + 2, _("Free Space"), Q_COLOR_MENU_COMMAND);
                screen_put_color_printf(Q_COLOR_MENU_TEXT, "  %'-lu k", free_kbytes);
                i++;
        }
        screen_put_color_str_yx(window_top + i, window_left + 2, "A", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Ascii"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "K", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Kermit"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "X", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Xmodem"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "C", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Xmodem CRC"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "R", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Xmodem Relaxed"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "O", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Xmodem-1K"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "Y", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Ymodem Batch"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "Z", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Zmodem Batch"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "F", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Xmodem-1K/G"), Q_COLOR_MENU_TEXT);
        i++;
        screen_put_color_str_yx(window_top + i, window_left + 2, "G", Q_COLOR_MENU_COMMAND);
        screen_put_color_str(_(" - Ymodem/G Batch"), Q_COLOR_MENU_TEXT);
        i++;

        /* Prompt */
        screen_put_color_str_yx(window_top + i, window_left + 2, _("Your Choice ? "), Q_COLOR_MENU_COMMAND);
...
/*
 * protocol_pathdialog_refresh - This screen handler is unique in that it blocks
 *                               the main loop.
 */
void protocol_pathdialog_refresh() {
        ...
        case Q_PROTOCOL_ZMODEM:
                /*
                 * Kermit, Ymodem and Zmodem get the filename from the
                 * download itself, so prompt for a pathname rather
                 * than a filename.
                 */
                if (q_program_state == Q_STATE_DOWNLOAD_PATHDIALOG) {
                        ...
                        batch_upload_file_list = batch_entry_window(get_option(Q_OPTION_UPLOAD_DIR), Q_TRUE);
                        if (batch_upload_file_list != NULL) {
                                /* Begin uploading */
                                batch_upload_file_list_i = 0;
                        ...
                        /*
                         * I could let this fall out below, but that
                         * would be slightly confusing. This function
                         * is zig-zaggy enough already.
                         */
                        return;
                }
                break;
...
/*
 * protocol_pathdialog_keyboard_handler
 */
void protocol_pathdialog_keyboard_handler(const int keystroke, const int flags) {
        /* This function gets called after protocol_pathdialog_refresh(),
           so it needs to exist, but it doesn't actually do anything.
           protocol_pathdialog_refresh() calls functions in forms.c
           that do their own internal keyboard handling. */
...
void protocol_transfer_refresh() {
        ...
        if (q_transfer_stats.bytes_transfer > 0) {
                remaining_time = (q_transfer_stats.bytes_total - q_transfer_stats.bytes_transfer) * transfer_time / q_transfer_stats.bytes_transfer;
        } else {
                remaining_time = 0;
        }
        if ((q_transfer_stats.state == Q_TRANSFER_STATE_END) || (q_transfer_stats.state == Q_TRANSFER_STATE_FILE_DONE)) {
                remaining_time = 0;
        }
        ...
        if (((q_transfer_stats.state == Q_TRANSFER_STATE_END) || (q_transfer_stats.state == Q_TRANSFER_STATE_ABORT)) && (q_screen_dirty == Q_FALSE)) {
        ...
        /*
         * ASCII special case: we don't put up the dialog window, instead
         * we update the status line.
         */
        if (q_transfer_stats.protocol == Q_PROTOCOL_ASCII) {
                ...
                if (q_program_state == Q_STATE_DOWNLOAD) {
                        ...
                        status_string = _(" ASCII DOWNLOAD IN PROGRESS    ESC/`-Save and Exit ");

                } else {
                        if (q_transfer_stats.bytes_total == 0) {
                                /* If the file is 0, percent complete is always 100. */
                                percent_complete = 100;
                        } else if (q_transfer_stats.bytes_transfer == q_transfer_stats.bytes_total) {
                                /* If file is complete percent complete is 100. */
                                percent_complete = 100;
                        } else {
                                /* Else do a true percent complete calculation */
                                if (q_transfer_stats.bytes_total > 0) {
                                        percent_complete = (q_transfer_stats.bytes_transfer * 100) / q_transfer_stats.bytes_total;
                                } else {
                                        percent_complete = 0;
                                }
                        }

                        /* " Uploading %s  Sent = %d    Complete = %d%%  ESC/`-Terminate " */
                        sprintf(status_buffer, _(" Uploading %s  Sent = %lu    Complete = %d%%   ESC/`-Terminate "), q_transfer_stats.filename, q_transfer_stats.bytes_transfer, percent_complete);
        ...
        if (q_screen_dirty == Q_FALSE) {
                ...
                screen_put_color_str_yx(window_top + 6, window_left + 51, _("Time Elapsed "), Q_COLOR_MENU_TEXT);
                screen_put_color_str(time_elapsed_string, Q_COLOR_MENU_COMMAND);
                screen_put_color_str_yx(window_top + 7, window_left + 51, _("++ Remaining "), Q_COLOR_MENU_TEXT);
                screen_put_color_str(remaining_time_string, Q_COLOR_MENU_COMMAND);

                /* Batch time */
                if (q_program_state == Q_STATE_UPLOAD_BATCH) {
                        /* Batch times */
                        screen_put_color_str_yx(window_top + window_height - 1 - 2, window_left + 2, _("Batch Time Elapsed "), Q_COLOR_MENU_TEXT);
                        screen_put_color_str_yx(window_top + window_height - 1 - 2, window_left + 21, batch_time_elapsed_string, Q_COLOR_MENU_COMMAND);
                        screen_put_color_str_yx(window_top + window_height - 1 - 2, window_left + 49, _("++ Remaining "), Q_COLOR_MENU_TEXT);
                        screen_put_color_str_yx(window_top + window_height - 1 - 2, window_left + 62, batch_remaining_time_string, Q_COLOR_MENU_COMMAND);
                }
                return;
        }
        ...
        if (q_program_state == Q_STATE_DOWNLOAD) {
                ...
                if (q_transfer_stats.protocol == Q_PROTOCOL_KERMIT) {
                        status_string = _(" Download in Progress   S-Skip File   ESC/`-Cancel Transfer ");
                } else {
                ...
                message = _("Upload Status");
                if (q_transfer_stats.protocol == Q_PROTOCOL_KERMIT) {
                        status_string = _(" Batch Upload in Progress   S-Skip File   ESC/`-Cancel Transfer ");
                } else {
                        status_string = _(" Batch Upload in Progress   ESC/`-Cancel Transfer ");
                }
        }
        ...
        screen_put_color_str_yx(window_top + 3, window_left + 2, _("File "), Q_COLOR_MENU_TEXT);
        screen_put_color_str(q_transfer_stats.filename, Q_COLOR_MENU_COMMAND);
        screen_put_color_str_yx(window_top + 1, window_left + 27, _("Protocol "), Q_COLOR_MENU_TEXT);
        screen_put_color_str(q_transfer_stats.protocol_name, Q_COLOR_MENU_COMMAND);

        screen_put_color_str_yx(window_top + 4, window_left + 2, _("Path "), Q_COLOR_MENU_TEXT);
        screen_put_color_str(q_transfer_stats.pathname, Q_COLOR_MENU_COMMAND);

        /* Bytes and blocks total fields */
        screen_put_color_str_yx(window_top + 6, window_left + 2, _("Bytes Total "), Q_COLOR_MENU_TEXT);
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-lu", q_transfer_stats.bytes_total);
        screen_put_color_str_yx(window_top + 6, window_left + 27, _("Blocks Total "), Q_COLOR_MENU_TEXT);
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-lu", q_transfer_stats.blocks);
        ...
        if (q_program_state == Q_STATE_DOWNLOAD) {
                screen_put_color_str_yx(window_top + 7, window_left + 2, _("Bytes Rcvd  "), Q_COLOR_MENU_TEXT);
        } else {
                screen_put_color_str_yx(window_top + 7, window_left + 2, _("Bytes Sent  "), Q_COLOR_MENU_TEXT);
        }
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-lu", q_transfer_stats.bytes_transfer);
        if (q_program_state == Q_STATE_DOWNLOAD) {
                screen_put_color_str_yx(window_top + 7, window_left + 27, _("Blocks Rcvd  "), Q_COLOR_MENU_TEXT);
        } else {
                screen_put_color_str_yx(window_top + 7, window_left + 27, _("Blocks Sent  "), Q_COLOR_MENU_TEXT);
        }
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-lu", q_transfer_stats.blocks_transfer);

        /* Block size, error count, and efficiency */
        screen_put_color_str_yx(window_top + 8, window_left + 2, _("Error Count "), Q_COLOR_MENU_TEXT);
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-lu", q_transfer_stats.error_count);
        screen_put_color_str_yx(window_top + 8, window_left + 27, _("Block Size   "), Q_COLOR_MENU_TEXT);
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-lu", q_transfer_stats.block_size);
        ...
        screen_put_color_str_yx(window_top + 8, window_left + 51, _("Efficiency   "), Q_COLOR_MENU_TEXT);
        ...
        if (q_status.serial_open == Q_TRUE) {
                ...
                if (q_serial_port.dce_baud > 0) {
                        screen_put_color_printf(Q_COLOR_MENU_COMMAND, " %6.2f%%", ((float)cps * bits_per_byte * 100.0) / (float)q_serial_port.dce_baud);
                } else {
        ...
        /* Last message */
        screen_put_color_str_yx(window_top + 10, window_left + 2, _("Status Msgs "), Q_COLOR_MENU_TEXT);
        ...
        if ((q_transfer_stats.state == Q_TRANSFER_STATE_END) || (q_transfer_stats.state == Q_TRANSFER_STATE_FILE_DONE)) {
                ...
                percent_complete = 0;
        } else if ((q_program_state == Q_STATE_DOWNLOAD) && ((q_transfer_stats.protocol == Q_PROTOCOL_XMODEM) ||
                   (q_transfer_stats.protocol == Q_PROTOCOL_XMODEM_RELAXED) ||
                   (q_transfer_stats.protocol == Q_PROTOCOL_XMODEM_CRC) ||
                   (q_transfer_stats.protocol == Q_PROTOCOL_XMODEM_1K) ||
                   (q_transfer_stats.protocol == Q_PROTOCOL_XMODEM_1K_G)
        )) {
                /* Xmodem does not report file size on downloads, percent complete is always 0 */
                percent_complete = 0;
        ...
        screen_put_color_str_yx(window_top + 11, window_left + 2, _("Completion  "), Q_COLOR_MENU_TEXT);
        screen_move_yx(window_top + 11, window_left + 14);
        screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-3u%%   ", percent_complete);
        ...
        if (q_program_state == Q_STATE_UPLOAD_BATCH) {
                ...
                } else if (q_transfer_stats.batch_bytes_transfer + q_transfer_stats.bytes_transfer >= q_transfer_stats.batch_bytes_total) {
                        batch_percent_complete = 100;
                } else if (q_transfer_stats.batch_bytes_total == 0) {
                        /* If we ever send a batch of 0-byte files... */
                        batch_percent_complete = 0;
                } else {
                        batch_percent_complete = ((q_transfer_stats.batch_bytes_transfer + q_transfer_stats.bytes_transfer) * 100) / q_transfer_stats.batch_bytes_total;
                }
                ...
                screen_put_color_hline_yx(window_top + window_height - 1 - 3, window_left + 1, cp437_chars[Q_WINDOW_TOP], window_length - 2, Q_COLOR_WINDOW_BORDER);
                screen_put_color_char_yx(window_top + window_height - 1 - 3, window_left + 0, cp437_chars[Q_WINDOW_LEFT_TEE], Q_COLOR_WINDOW_BORDER);
                screen_put_color_char_yx(window_top + window_height - 1 - 3, window_left + window_length - 1, cp437_chars[Q_WINDOW_RIGHT_TEE], Q_COLOR_WINDOW_BORDER);
                ...
                screen_put_color_printf_yx(window_top + window_height - 1 - 3, window_left + message_left, Q_COLOR_WINDOW_BORDER, " %s ", message);
                ...
                /* Progress bar */
                screen_put_color_str_yx(window_top + window_height - 1 - 1, window_left + 2, _("Completion  "), Q_COLOR_MENU_TEXT);
                screen_move_yx(window_top + window_height - 1 - 1, window_left + 14);
                screen_put_color_printf(Q_COLOR_MENU_COMMAND, "%-3u%%   ", batch_percent_complete);
