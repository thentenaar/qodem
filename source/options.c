...
/* Options structure.  I think this is the most useless comment so far. :-) */
...
#else
        {Q_OPTION_SSH_KNOWNHOSTS, NULL, "knownhosts_file", "$HOME/.ssh/known_hosts", ""
#endif /* Q_PDCURSES_WIN32 */
...
/*
 * substitute_string - replace all instances of "pattern" in "original"
 *                     with "new_string", returning a newly-malloc'd
 *                     string.
 */
char * substitute_string(const char * original, const char * pattern, const char * new_string) {
        int i;
        ...
        return_string = (char *)Xmalloc(1 + strlen(original) + (i * (strlen(new_string) - strlen(pattern))), __FILE__, __LINE__);
        memset(return_string, 0, 1 + strlen(original) + (i * (strlen(new_string) - strlen(pattern))));
        /* Cast to avoid compiler warning */
        ...
        while (i>0) {
                ...
                strncpy(return_string + return_string_place, old_place, current_place - old_place);
                return_string_place += current_place - old_place;
                old_place = current_place;
                /* Copy new string */
                strncpy(return_string + return_string_place, new_string, strlen(new_string));
        ...
        strncpy(return_string + return_string_place, current_place, original + strlen(original) - current_place);

        return return_string;
} /* ---------------------------------------------------------------------- */

/*
 * substitute_wcs - replace all instances of "pattern" in "original"
 *                  with "new_string", returning a newly-malloc'd
 *                  string.
 */
wchar_t * substitute_wcs(const wchar_t * original, const wchar_t * pattern, const wchar_t * new_string) {
        int i;
        /* Cast to avoid compiler warning */
        ...
        /* i contains the # of occurrences of pattern. */
        return_string = (wchar_t *)Xmalloc((sizeof (wchar_t)) * (1 + wcslen(original) + (i * (wcslen(new_string) - wcslen(pattern)))), __FILE__, __LINE__);
        wmemset(return_string, 0, 1 + wcslen(original) + (i * (wcslen(new_string) - wcslen(pattern))));
        /* Cast to avoid compiler warning */
        ...
        while (i>0) {
                ...
                /* Copy part before pattern */
                wcsncpy(return_string + return_string_place, old_place, current_place - old_place);
                return_string_place += current_place - old_place;
                old_place = current_place;
                /* Copy new string */
                wcsncpy(return_string + return_string_place, new_string, wcslen(new_string));
        ...
        /* Copy terminating non-pattern part */
        wcsncpy(return_string + return_string_place, current_place, original + wcslen(original) - current_place);

        return return_string;
} /* ---------------------------------------------------------------------- */

/*
 * substitute_wcs_half - replace all instances of "pattern" in "original"
 *                       with "new_string", returning a newly-malloc'd
 *                       string.
 */
char * substitute_wcs_half(const char * original, const char * pattern, const wchar_t * new_string) {
        int i;
        /* Cast to avoid compiler warning */
        char * current_place = (char *)original;
        char * old_place;
        ...
        /* i contains the # of occurrences of pattern. */
        return_string = (char *)Xmalloc(1 + strlen(original) + (i * (strlen(new_str) - strlen(pattern))), __FILE__, __LINE__);
        memset(return_string, 0, 1 + strlen(original) + (i * (strlen(new_str) - strlen(pattern))));
        /* Cast to avoid compiler warning */
        current_place = (char *)original;
        old_place = current_place;
        return_string_place = 0;
...
/*
 * Retrieve an option description string.
 */
...
/*
 * Retrieve an option default value.
 */
...
static Q_BOOL save_options(const char * filename) {
        ...
        if (file == NULL) {
                fprintf(stderr, _("Error opening file \"%s\" for writing: %s"), filename, strerror(errno));
                return Q_FALSE;
        }

        current_option = options;
        while (current_option->option != Q_OPTION_NULL) {
                rc = fwrite(current_option->comment, strlen(current_option->comment), 1, file);
                if (rc == -1) {
                        fprintf(stderr, _("Error writing to file \"%s\": %s"), filename, strerror(errno));
                        return Q_FALSE;
                }
                rc = fwrite("\n###", 4, 1, file);
                if (rc == -1) {
                ...
                rc = fwrite(current_option->default_value, strlen(current_option->default_value), 1, file);
                if (rc == -1) {
                        fprintf(stderr, _("Error writing to file \"%s\": %s"), filename, strerror(errno));
                        return Q_FALSE;
                }
                if (strncmp(current_option->value, current_option->default_value, strlen(current_option->default_value)) == 0) {
                        rc = fwrite("\n### ", 5, 1, file);
                        if (rc == -1) {
                                fprintf(stderr, _("Error writing to file \"%s\": %s"), filename, strerror(errno));
                                return Q_FALSE;
                ...
                rc = fwrite(current_option->name, strlen(current_option->name), 1, file);
                if (rc == -1) {
                        fprintf(stderr, _("Error writing to file \"%s\": %s"), filename, strerror(errno));
                        return Q_FALSE;
                }
                ...
                rc = fwrite(current_option->value, strlen(current_option->value), 1, file);
                if (rc == -1) {
                        fprintf(stderr, _("Error writing to file \"%s\": %s"), filename, strerror(errno));
                        return Q_FALSE;
                }
...
static void set_option(struct option_struct * option, char * value) {
        ...
        while (((isspace(new_value[0])) || (new_value[0] == '=')) && (strlen(new_value) > 0)) {
                new_value++;
        }
...
static void load_options_from_file(const char * filename) {
        ...
        if (file == NULL) {
                fprintf(stderr, _("Error opening file \"%s\" for reading: %s"), filename, strerror(errno));
                return;
        }
        ...
        while (!feof(file)) {
                ...
                if (fgets(line, sizeof(line), file) == NULL) {
                        if (!feof(file)) {
                                fprintf(stderr, _("Error reading from file \"%s\": %s"), filename, strerror(errno));
                        }
                        break;
                }
                ...
                while ((strlen(line_begin) > 0) && isspace(line_begin[strlen(line_begin)-1])) {
                ...
                while (current_option->option != Q_OPTION_NULL) {
                        if (strstr(line, current_option->name) == line) {
                                if ((line[strlen(current_option->name)] == '=') ||
                                        ...
                                        /* Valid option */
                                        set_option(current_option, line + strlen(current_option->name));
                                }
                        }
...
/*
 * Get the full filename to "$HOME/.qodem/qodemrc"
 */
...
void load_options() {
        ...
        char * filenames[] = {
                ...
                /* List $HOME last so it overrides everything else */
#ifdef Q_PDCURSES_WIN32
        ...
        while (current_option->option != Q_OPTION_NULL) {
                ...
                current_option->value = Xstrdup(current_option->default_value, __FILE__, __LINE__);
...
#ifdef Q_PDCURSES_WIN32
                working_dir = substitute_string("$HOME\\qodem\\prefs", "$HOME", env_string);
#else
...
"The default directory is %s.  Should I create this directory now? [Y/n] "), working_dir);
                        i = getchar();
...
"when you load key bindings, phone books, etc.\n"), working_dir);
        ...
        /* Sustitute for $HOME */
#ifdef Q_PDCURSES_WIN32
        substituted_filename = substitute_string("$HOME/qodem/prefs/qodemrc.txt", "$HOME", env_string);
#else
...
#ifdef ASK_TO_CREATE
                printf(_(""
"Qodem needs to set the LANG environment variable when it connects to remote systems.\n"
"The current value for UTF-8 systems is '%s'.  Press Enter to use this value, or type\n"
"in a new value:  "), lang_default);
                ...
                printf(_(""
"Qodem needs to set the LANG environment variable when it connects to remote systems.\n"
"The current value for non-UTF-8 systems is '%s'.  Press Enter to use this value, or type\n"
"in a new value:  "), lang_default);
        ...
        /* Sustitute for $HOME */
        substituted_filename = substitute_string("$HOME/.qodem/scripts/script.stderr", "$HOME", env_string);
        ...
        i = 0;
        for (current_filename = filenames[i]; current_filename != NULL; i++, current_filename = filenames[i]) {
                /* Sustitute for $HOME */
                substituted_filename = substitute_string(current_filename, "$HOME", env_string);

                /* Save that last filename so we can use it for Alt-N Configuration later */
                if (filenames[i + 1] == NULL) {
                        sprintf(home_directory_options_filename, "%s", substituted_filename);
                }

                /*
                 * Check existence of file.  For the Windows case,
        ...
        working_dir = substitute_string(get_option(Q_OPTION_WORKING_DIR), "$HOME", env_string);
        if (directory_exists(working_dir) == Q_FALSE) {
#ifdef ASK_TO_CREATE
                printf(_(""
"Qodem needs to create a directory to store user files such\n"
"as downloaded and uploaded files, session logs, capture files, etc.\n"
"The default directory is %s.  Should I create this directory now? [Y/n] "), working_dir);
                i = getchar();
                if ((tolower(i) == 'y') || (i == '\r') || (i == '\n')) {
#endif /* ASK_TO_CREATE */
...
"when you download files, enable capture/log, etc.\n"), working_dir);
        ...
        working_dir = substitute_string(get_option(Q_OPTION_HOST_DIR), "$HOME", env_string);
        if (directory_exists(working_dir) == Q_FALSE) {
#ifdef ASK_TO_CREATE
                printf(_(""
"Qodem needs to create a directory to store host mode files such\n"
"as downloaded and uploaded files, email messages, etc.\n"
"The default directory is %s.  Should I create this directory now? [Y/n] "), working_dir);
                i = getchar();
                if ((tolower(i) == 'y') || (i == '\r') || (i == '\n')) {
#endif /* ASK_TO_CREATE */
...
"perform uploads and downloads or messages.\n"), working_dir);
        ...
        /*
         * Special-case options.  For each one, reset to default and then re-load from file.
         */
        ...
        if (strcasecmp(get_option(Q_OPTION_KERMIT_UPLOADS_FORCE_BINARY), "false") == 0) {
                q_status.kermit_uploads_force_binary = Q_FALSE;
        }
        q_status.kermit_downloads_convert_text  = Q_FALSE;
        if (strcasecmp(get_option(Q_OPTION_KERMIT_DOWNLOADS_CONVERT_TEXT), "true") == 0) {
...
/* Reset capture type value to whatever is in the options file */
...
/* Reset screen dump type value to whatever is in the options file */
...
/* Reset scrollback save type value to whatever is in the options file */
