...
void load_translate_tables() {
        ...
        while (!feof(file)) {
                ...
                *value = 0;
                ...
                while ((isspace(key[strlen(key) - 1])) && (strlen(key) > 0)) {
                        key[strlen(key) - 1] = 0;
                }
...
static void save_translate_tables() {
        ...
        if (file == NULL) {
                snprintf(notify_message, sizeof(notify_message), _("Error opening file \"%s\" for writing: %s"), TRANSLATE_TABLE_FILENAME, strerror(errno));
...
/*
 * Create empty translate tables file in the data directory ($HOME/.qodem)
 */
...
void translate_table_menu_refresh() {
        ...
        screen_put_color_str_yx(window_top + 2, window_left + 2, _("Select Table to Edit"), Q_COLOR_MENU_TEXT);

        screen_put_color_str_yx(window_top + 4, window_left + 7, "1", Q_COLOR_MENU_COMMAND);
        screen_put_color_printf(Q_COLOR_MENU_TEXT, " -  %s", _("INPUT"));
        screen_put_color_str_yx(window_top + 5, window_left + 7, "2", Q_COLOR_MENU_COMMAND);
        screen_put_color_printf(Q_COLOR_MENU_TEXT, " -  %s", _("OUTPUT"));

        /* Prompt */
        screen_put_color_str_yx(window_top + 7, window_left + 2, _("Your Choice ? "), Q_COLOR_MENU_COMMAND);
...
/*
 * translate_table_menu_keyboard_handler
 */
void translate_table_menu_keyboard_handler(const int keystroke, const int flags) {
...
/*
 * translate_table_editor_keyboard_handler
 */
void translate_table_editor_keyboard_handler(const int keystroke, const int flags) {
        int row;
        int col;
        int new_keystroke;
        wchar_t * value;
        ...
        if (selected_entry < 10) {
                edit_table_entry_window = screen_subwin(1, 3, window_top + window_height - 3, window_left + 49);
        } else if (selected_entry < 100) {
                edit_table_entry_window = screen_subwin(1, 3, window_top + window_height - 3, window_left + 50);
        } else {
                edit_table_entry_window = screen_subwin(1, 3, window_top + window_height - 3, window_left + 51);
        }
        ...
        edit_table_entry_form = fieldset_malloc(&edit_table_entry_field, 1, edit_table_entry_window);

        screen_put_color_printf_yx(window_top + window_height - 3, window_left + 25, Q_COLOR_MENU_COMMAND, _("Enter new value for %d >"), selected_entry);

        snprintf(buffer, sizeof(buffer), "%d", editing_table->map_to[selected_entry]);
        ...
        /*
         * Render the field.  Must be called AFTER screen_flush() to put the cursor on the
         * right spot.
         */
        q_cursor_on();
...
void translate_table_editor_refresh() {
        ...
        screen_put_color_printf_yx(window_top, window_left + title_left, Q_COLOR_WINDOW_BORDER, " %s ", title);
...
" ARROWS-Movement   S-SWAP   SPACEBAR-Change   F10-Save   ESC/`-Exit ");
        ...
        screen_put_color_str_yx(window_top + 1, window_left + 21, _("In Character | |  Out Character | |"), Q_COLOR_MENU_TEXT);
        screen_put_color_char_yx(window_top + 1, window_left + 21 + 14, cp437_chars[selected_entry & 0xFF], Q_COLOR_MENU_COMMAND);
        screen_put_color_char_yx(window_top + 1, window_left + 21 + 33, cp437_chars[editing_table->map_to[selected_entry] & 0xFF], Q_COLOR_MENU_COMMAND);
        ...
        for (; i<end_i; i++) {
                ...
                if (i == selected_entry) {
                        screen_put_color_printf_yx(window_top + 2 + row, window_left + 3 + (col * 9), Q_COLOR_MENU_COMMAND, "[%3d-%3d]", i, editing_table->map_to[i]);
                } else {
                        screen_put_color_printf_yx(window_top + 2 + row, window_left + 3 + (col * 9), Q_COLOR_MENU_TEXT, " %3d-%3d ", i, editing_table->map_to[i]);
                }
        }
